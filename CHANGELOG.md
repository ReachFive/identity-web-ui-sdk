...
- `theme.socialButton` colours now apply, falling back to the provider's brand colour
...
- The WebAuthn registered-devices list is rebuilt on the shared `Item` primitives
...
- Password rules: a satisfied rule is shown in the success colour instead of the primary color
...
## Changed
- adapt wording accordingly with login type allowed configuration
...
- Use theme colors in default button component
- Use theme colors in phone number country select arrow color
- Set remember me checkbox optional

### Fixed

- Fix webauthn paragraph text wrapping
- Create new i18next instance per I18nProvider

## [1.39.0] - 2025-11-13

### Added

- Allow to override i18n messages by language
- Add asterisk when a consent is required

### Changed

- Replace year input field with select dropdown in date field
