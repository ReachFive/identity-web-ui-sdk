...
type PhoneNumberRegisteringCredentialFormData = {
    phoneNumber: string
    trustDevice: boolean
}
...
const MainView = ({
    ...
    const onPhoneNumberRegistering = (
    ...
    const phoneNumberCredentialRegistered =
        credentials.find<MFA.PhoneCredential>(
            (credential): credential is MFA.PhoneCredential =>
                ...
                {config.mfaEmailEnabled && !isEmailCredentialRegistered && (
                    <div>
                        ...
                        <EmailRegisteringCredentialForm
                            ...
                            onSuccess={(
                                data: StartMfaEmailRegistrationResponse
                            ) =>
                                goTo<VerificationCodeViewState>(
                                    'verification-code',
                                    {
                                        ...data,
                ...
                {config.mfaSmsEnabled && !isPhoneCredentialRegistered && (
                    <div>
                        ...
                        <PhoneNumberRegisteringCredentialForm
                            ...
                            onSuccess={(
                                data: StartMfaPhoneNumberRegistrationResponse
                            ) =>
                                goTo<VerificationCodeViewState>(
                                    'verification-code',
                                    {
                ...
                {showRemoveMfaCredentials &&
                    ...
                    isEmailCredentialRegistered && (
                        <div>
                            {showIntro && (
                                <Intro>
                                    {i18n('mfa.email.remove.explain')}
                                </Intro>
                            )}
                            <EmailCredentialRemovalForm
                                handler={onEmailRemoval}
                                onSuccess={() =>
                                    goTo<CredentialRemovedViewState>(
                                        'credential-removed',
                ...
                {showRemoveMfaCredentials &&
                    ...
                    phoneNumberCredentialRegistered && (
                        <div>
                            {showIntro && (
                                <Intro>
                                    {i18n('mfa.phoneNumber.remove.explain')}
                                </Intro>
                            )}
                            <PhoneNumberCredentialRemovalForm
                                handler={() =>
                                    onPhoneNumberRemoval({
                                        ...phoneNumberCredentialRegistered,
                                    })
                                }
                                onSuccess={() =>
...
const VerificationCodeView = ({
    ...
    const onCredentialRegistered = () => {
        onSuccess({ name: 'credential_registered' })
        goTo<CredentialRegisteredViewState>('credential-registered', {
            registrationType,
        })
    }
    ...
    return (
        <div>
            {showIntro && status === 'email_sent' && (
                <Intro>{i18n('mfa.verify.email')}</Intro>
            )}
            {status === 'email_sent' && (
            ...
            {showIntro && status === 'sms_sent' && (
                <Intro>{i18n('mfa.verify.sms')}</Intro>
            )}
            {status === 'sms_sent' && (
                <VerificationCodeForm
                    displayTrustDevice={allowTrustDevice && config.rbaEnabled}
