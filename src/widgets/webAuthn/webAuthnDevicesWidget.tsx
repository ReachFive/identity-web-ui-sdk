...
const DevicesList = ({ devices, removeWebAuthnDevice }: DevicesListProps) => {
    ...
    return (
        <DevicesListWrapper>
            ...
            <div>
                {devices.map((device) => {
                    const { id, friendlyName, createdAt, lastUsedAt, aaguid } =
                        device
                        ...
                        <Card key={id} data-testid="device">
                            <CardContent>
                                ...
                                <CardText>
                                    ...
                                    {provider && (
                                        <div data-testid="device-provider">
                                            {provider}
                                        </div>
                                    )}
                                    {createdAt && (
...
function WebAuthnDevices({
    ...
    const removeWebAuthnDevice = (deviceId: string) => {
        ...
        return coreClient
            ...
            .then(() => {
                onSuccess({ name: 'web_authn_device_removed', deviceId })
                return coreClient
...
export default createWidget<WebAuthnWidgetProps, WebAuthnDevicesProps>({
    ...
    prepare: (options, { apiClient, config }) => {
        ...
        if (!accessToken) {
            const error = new UserError(
                'You must be logged in to manage the FIDO2 devices.'
            )
            options.onError?.(error)
            throw error
        }
...
>([
    [
        ...
        ['Google Password Manager', GooglePasswordManager],
    ],
    ...
    [
        ...
        ['Thales Bio Android SDK', Thales],
    ],
    [
        'cd69adb5-3c7a-deb9-3177-6800ea6cb72a',
        ['Thales PIN Android SDK', Thales],
    ],
