import React, { ComponentType, type PropsWithChildren, ReactNode, Suspense, createContext, useContext } from 'react';
import { ErrorResponse, type ConsentVersions, type Client as CoreClient, type Config as CoreConfig } from '@reachfive/identity-core';
import { QueryClient, QueryClientProvider, useSuspenseQuery, useSuspenseQueries } from '@tanstack/react-query';
 ...
 * ```
 */
export function ReachfiveProvider({ children, client, config: coreConfig, fallback }: PropsWithChildren<ReachfiveProviderProps>) {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <ErrorBoundary fallbackRender={({error}) => <ErrorText>{adaptError(error)}</ErrorText>}>
...
function ConfigProvider({ children: Children, client, coreConfig }: {
    ...
    queries: [
      {
        ...
        queryFn: async () => {
          const consentsResponse = await fetch(`https://${coreConfig.domain}/identity/v1/config/consents?lang=${language}`)
