...
function Consumer() {
    ...
    return (
        <>
            <p>{config.domain}</p>
            <p>{config.language}</p>
            <p>providers: {config.socialProviders.map(provider => <span key={provider}>{provider}</span>)}</p>
            <dl>
...
describe('ReachfiveProvider', () => {
    const getSessionInfo = jest.fn<Client['getSessionInfo']>().mockResolvedValue({ isAuthenticated: true })
    ...
    const remoteSettingsSpy = jest.spyOn(client, 'remoteSettings', 'get').mockResolvedValue(defaultConfig)
