...
describe('Snapshot', () => {
    const generateSnapshot = (options: Parameters<typeof passwordlessWidget>[0] = {}, config: Partial<Config> = {}) => async () => {
        // @ts-expect-error partial Client
        const apiClient: Client = {}

        const widget = await passwordlessWidget(options, {config: { ...defaultConfig, ...config }, apiClient, defaultI18n })
...
describe('DOM testing', () => {
    ...
    const generateComponent = async (options: Parameters<typeof passwordlessWidget>[0] = {}, config: Partial<Config> = {}) => {
        // @ts-expect-error partial Client
        const apiClient: Client = {
            startPasswordless,
            verifyPasswordless,
