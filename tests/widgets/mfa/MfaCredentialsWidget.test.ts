    ...
    const generateSnapshot =
        ...
        async () => {
            ...
            await waitFor(async () => {
                ...
                await waitFor(() =>
                    expect(apiClient.listMfaCredentials).toHaveBeenCalled()
                )
...
describe('DOM testing', () => {
    ...
    const startMfaEmailRegistration =
        jest.fn<Client['startMfaEmailRegistration']>()
    const startMfaPhoneNumberRegistration =
        jest.fn<Client['startMfaPhoneNumberRegistration']>()
    const verifyMfaEmailRegistration =
        jest.fn<Client['verifyMfaEmailRegistration']>()
    const verifyMfaPhoneNumberRegistration =
        jest.fn<Client['verifyMfaPhoneNumberRegistration']>()
    ...
    const generateComponent = async (
        ...
        // @ts-expect-error partial Client
        const apiClient: Client = {
            listMfaCredentials: listMfaCredentials.mockResolvedValue({
                credentials,
            }),
            startMfaEmailRegistration:
                startMfaEmailRegistration.mockResolvedValue({
                    status: 'email_sent',
                }),
            startMfaPhoneNumberRegistration:
                startMfaPhoneNumberRegistration.mockResolvedValue({
                    status: 'sms_sent',
                }),
            verifyMfaEmailRegistration:
                verifyMfaEmailRegistration.mockResolvedValue(),
    ...
    describe('mfaCredentials', () => {
        ...
        test('register email', async () => {
            ...
            await waitFor(async () => {
                expect(
                    screen.queryByText('mfa.verify.email')
                ).toBeInTheDocument()
            })

            const verificationCodeInput =
                screen.getByTestId('verification_code')
            ...
            await waitFor(async () => {
                expect(
                    screen.queryByText('mfa.email.registered')
                ).toBeInTheDocument()
        ...
        test('register phone number', async () => {
            ...
            const phoneNumberButton = screen.getByText(
            ...
            await waitFor(async () => {
                expect(
                    screen.queryByText('mfa.phoneNumber.registered')
                ).toBeInTheDocument()
                expect(onSuccess).toBeCalledWith(
                    expect.objectContaining({
                        name: 'credential_registered',
