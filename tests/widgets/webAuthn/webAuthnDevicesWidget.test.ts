...
describe('DOM testing', () => {
    ...
    describe('webAuthn', () => {
        test('WebAuthn feature is not available', async () => {
            await expect(
                generateComponent({}, { webAuthn: false })
            ).rejects.toThrow(
        ...
        test('default', async () => {
            ...
            const friendlyNameInput = screen.getByLabelText(
            ...
            expect(addNewWebAuthnDevice).toBeCalledWith(
                accessToken,
                'myNewDevice'
            )
            ...
            const deviceNames = screen
                .queryAllByTestId('device-name')
                .map((el) => el.textContent)
            expect(deviceNames).toEqual(['myOldDevice', 'myNewDevice'])

            const oldDevice = devicesAfterAdd.find(
                (el) => el.textContent === 'myOldDevice'
            )
            const removeOldDevice = oldDevice?.querySelector(
            ...
            expect(removeWebAuthnDevice).toBeCalledWith(
                accessToken,
                'myOldDevice'
            )
        ...
        test('list devices api failure', async () => {
            ...
            await expect(generateComponent({})).rejects.toMatch(
                'Unexpected error'
            )
